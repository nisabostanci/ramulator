#ifndef __HYBRIDMEMORY_H
#define __HYBRIDMEMORY_H
...
template <class T,class T2, template<typename> class Controller = Controller >
class HybridMemory : public MemoryBase
{
protected:
...
public:
    ...
    HybridMemory(const Config& configs, vector<Controller<T>*> dram_ctrls, vector<Controller<T2>*> nvm_ctrls)
      : dram_ctrls(dram_ctrls),
          nvm_ctrls(nvm_ctrls),
          spec(dram_ctrls[0]->channel->spec),
          nvm_spec(nvm_ctrls[0]->channel->spec),
          ...
          addr_bits_nvm(int(T2::Level::MAX))
    {
        ...
        int *sz_nvm = nvm_spec->org_entry.count;
        assert((sz_nvm[0] & (sz_nvm[0] - 1)) == 0);
        assert((sz_nvm[1] & (sz_nvm[1] - 1)) == 0);
        ...
        max_address_nvm = nvm_spec->channel_width / 8;

        for (unsigned int lev = 0; lev < addr_bits.size(); lev++) {
          addr_bits[lev] = calc_log2(sz[lev]);
            max_address *= sz[lev];
        }
        for (unsigned int lev = 0; lev < addr_bits_nvm.size(); lev++) {
          addr_bits_nvm[lev] = calc_log2(sz_nvm[lev]);
            max_address_nvm *= sz_nvm[lev];
        }

        addr_bits[int(T::Level::MAX) - 1] -= calc_log2(spec->prefetch_size);
        addr_bits_nvm[int(T2::Level::MAX) - 1] -= calc_log2(nvm_spec->prefetch_size);
    ...
    ~HybridMemory()
    {
      for (auto ctrl: dram_ctrls) {
        delete ctrl;
      }
      for (auto ctrl: nvm_ctrls) {
        delete ctrl;
      }
        delete spec;
        delete nvm_spec;
    ...
    bool send(Request req)
    {
        ...
        if(dram_ctrls[req.addr_vec[0]]->enqueue(req)) {
